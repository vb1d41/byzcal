...
public final class ByzDate implements Comparable<ByzDate> {
    ...
    private static int toByzYear(int calYear, int calMonth) {
        return calYear + switch (Month.of(calMonth)) {
    ...
    private static ByzMonth toByzMonth(int calMonth) {
        var monthName = Month.of(calMonth).toString();
        return ByzMonth.valueOf(monthName);
    }

    private static int toCalMonth(ByzMonth byzMonth) {
        var monthName = byzMonth.toString();
        return Month.valueOf(monthName).getValue();
    }

    private static ByzDayOfWeek toByzDayOfWeek(int calDayOfWeek) {
        return switch (calDayOfWeek) {
            case Calendar.SUNDAY -> ByzDayOfWeek.LORDSDAY;
            case Calendar.MONDAY -> ByzDayOfWeek.SECOND;
            case Calendar.TUESDAY -> ByzDayOfWeek.THIRD;
            case Calendar.WEDNESDAY -> ByzDayOfWeek.FOURTH;
            case Calendar.THURSDAY -> ByzDayOfWeek.FIFTH;
            case Calendar.FRIDAY -> ByzDayOfWeek.PREPARATION;
            case Calendar.SATURDAY -> ByzDayOfWeek.SABBATH;
            default -> null;
        };
    }
